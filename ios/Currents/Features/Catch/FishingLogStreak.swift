...
struct BadgesGridView: View {
    ...
    var body: some View {
        let earned = allBadges.filter(\.earned).sorted { $0.rarity.sortOrder > $1.rarity.sortOrder }
        let locked = allBadges.filter { !$0.earned }.sorted { $0.rarity.sortOrder < $1.rarity.sortOrder }
